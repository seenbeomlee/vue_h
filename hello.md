...
- 장점: 쉬워요.
- 단점1: commit이 날아간다.
- 단점2: 강제 푸시가 필요하다(git push --force)
...
- 그렇게 하면 충돌을 막을 수 있다.
- 혹은 git-Bash에서 git push로 한꺼번에 할 수도 있음 (명령어는 구글 참조 ~)
...
- 혹은 commit -> 커밋 옵션 -> 마지막 커밋 정정 으로 sourcetree에서 실행

## stash

- 다른 브랜치로 체크아웃하기 전에 현재 작업내용을 저장하는 임시 젖장소
- 유용하니 잘 사용하자.

## 기타 주의 사항

- 코드를 남기려고 주석을 달지 말자. (어차피 커밋에 다 남는다)
- 커밋 메세지를 잘 쓰자.
